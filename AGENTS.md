...
## Project Context
This is the wrfxpy wildfire forecasting system, working on branch `james_ngfs`.
The primary entry point is src/ngfs/ngfs_start_2.py. The older src/ngfs_start.py
is a precursor and should not be treated as authoritative. Many files in src/ngfs
are untracked in git. Do not stage or commit any files without explicit instruction.
WRF-SFIRE model output files are scientific inputs — do not modify, move, or delete them.
...
What NOT to do
	...
	•	Do not change model parameters (especially defaults) without a stated goal and a baseline comparison.
...
- Do not execute Python scripts or submit jobs without explicit approval.
- Do not install packages into scientific conda environments without approval.
- The project runs in an HPC environment without root access. Do not suggest
  or attempt system-level changes.
- Data files produced by the wildfire model are read-only inputs. Do not 
  modify, move, or delete model output files.
- Do not assume a local display is available. Visualization code must 
  write to file, not call plt.show() or equivalent.
